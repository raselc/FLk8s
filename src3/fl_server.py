...
class FLServer(object):
                ...
                if len(self.current_round_all_client_updates) == len(self.client_sids_randomly_selected):
                    ...
                    if FLServer.NUM_CLIENTS_CONTACTED_PER_ROUND - result > \
                        ...
                        if self.current_round >= FLServer.MAX_NUM_ROUNDS:
                            ...
                            to_print = "\nExecution Time: " + str(ex_time) + " seconds"
    ...
    def get_correct_clients_then_train(self):
        ...
        for index, row in self.filter_client.iterrows():
            ...
            client_res_file = 'Datasets/ClientsDS/Resources-OLD/Client_' + str(row['Client_ID']) + '.csv'
    ...
    def check_client_resources(self, client_ID,cpu):
        ...
        res_df = pd.DataFrame(
            list(zip(data['data_size'], data['training_time'])),
            columns=['DataSetSize', 'TrainingTime'])

        suff_time = self.sufficient_time(data['client_ID'], res_df, data['train_and_valid_size'])
    ...
    def sufficient_time(self, client_ID, res_df, new_data):
        time_threshold = 30
        time_download = 0
        time_upload = 0
