...
def load_data_set(train_file_name, test_file_name, round_number):
    d = Path().resolve()#.parent.parent.parent
    with open(str(d) + r"/Datasets/kddcup.names.txt",
              'r') as infile:
    ...
    kdd_temp = pd.read_csv(str(d) + r"/Datasets/KDDTrainForDummy.csv", names=kdd_cols)
...
class FederatedClient(object):
    ...
    def __init__(self, client_file):
        ...
        self.sio = SocketIO(IPAddr, 5000, LoggingNamespace)
    ...
    def on_init(self, *args):
        ...
        d = Path().resolve()#.parent.parent.parent
        client_train_file = str(d) + r"/Datasets/ClientsDS/Unbalanced/Train/Client_" + str(
            model_config['real_client_ID'])
        client_test_file = str(d) + r"/Datasets/ClientsDS/Unbalanced/Test/Client_" + str(
            model_config['real_client_ID'])
        print('Train file', client_train_file)
        print('Test file', client_test_file)
        kdd_temp, kdd, kdd_t = load_data_set(client_train_file, client_test_file, model_config['round_number'])
        ...
        self.request_update(model_config)

    def request_update(self, req):
